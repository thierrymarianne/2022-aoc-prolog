...
loop(HowManyTimes, Matches, PreviousArrangement, PostMoveToDestinationArrangement) :-
    HowManyTimes > 0 -> (
        move_once(Matches, PreviousArrangement, _PostMoveToDestinationArrangement),
        loop(HowManyTimes - 1, Matches, _PostMoveToDestinationArrangement, PostMoveToDestinationArrangement)
    );
    PostMoveToDestinationArrangement = PreviousArrangement.

parse_moves(Stream, PreviousArrangement, Arrangement) :-
    (
        read_line_to_string(Stream, Line),
%        format("~s~n~n", Line),
        ...
        loop(TotalCratesToMove, Matches, PreviousArrangement, _Arrangement),
        parse_moves(Stream, _Arrangement, _)
    );
    (
        Arrangement = PreviousArrangement,
...
read_until_eos(Stream, PreviousArrangement, Arrangement) :-
    (
        not_eos(Stream) -> (
                \+ground(PreviousArrangement) -> (
                    parse_crates_arrangement(Stream, [[], [], []], Arrangement),
                    read_until_eos(Stream, Arrangement, _)
                );
                    parse_moves(Stream, PreviousArrangement, Arrangement)
        );
            TopCrates is PreviousArrangement,
            format('What crates end up at the top? ~q.~n~n', [TopCrates]),
            close(Stream),
