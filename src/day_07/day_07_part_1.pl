...
get_dir_filesize(Files, Dir, Filesize) :-
    format_absolute_path(Dir, DirAbsPath),
    get_dict(Dir, Files, Filesize) ->
    format("[INFO] ~q has size: ~d~n", [DirAbsPath, Filesize]);
...
execute_commands(Stream, WorkingDirectory, Filesizes, Directories, FilesOut, FilepathsIn, FilepathsOut) :-
    not_eos(Stream),
    read_line_to_string(Stream, Line) -> (
        change_directory(Line, WorkingDirectory, NextWorkingDirectory, Filesizes, Out),
        execute_commands(Stream, NextWorkingDirectory, Out, Directories, FilesOut, FilepathsIn, FilepathsOut);
        (
            list_files_in_directory(Line, WorkingDirectory, Directories, NextDirectories, Filesizes, FilesIn, FilepathsIn, Out),
            execute_commands(Stream, WorkingDirectory, FilesIn, NextDirectories, FilesOut, Out, FilepathsOut)
        )
    );
    ...
    pairs_keys_values(FoldedFilesizesPairs, _, FoldedFilesizesPairsValues),

    include(compliant_with_size_limit, FoldedFilesizesPairsValues, FilesizesOut),
    length(FilesizesOut, OutLength),
    length(FoldedFilesizesPairsValues, InLength),
    format("In: ~q ; Out: ~q~n~n", [InLength, OutLength]),

    sumlist(FilesizesOut, FilepathsOut),

    format("~n~nSolution is ~q~n~n", [FilepathsOut]).
...
find_solution(Solution) :-
    ...
    execute_commands(Stream, [], Filesizes, Directories, _, Filepaths, Solution),
